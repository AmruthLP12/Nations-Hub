...
const quizTypes = [
  ...
  },
  {
    key: "region",
    title: "Region Quiz",
    description: "Test your knowledge of world regions. Identify countries by their region.",
      ...
      <div className="main-body-container" style={{ flexDirection: "column", gap: "48px", width: "100%" }}>
        ...
        <div style={{ width: "100%", display: "flex", justifyContent: "center" }}>
          <div
            ...
            data-aos="fade-down"
          >
            <div className="ctd-info" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {quizTypes[0].icon}
                <p className="ctd-title" style={{ color: "#202D36", fontWeight: "bold", fontSize: 20 }}>Reach the Country</p>
              </div>
              <p className="ctd-desc" style={{ color: "#202D36" }}>{quizTypes[0].description}</p>
            </div>
          </div>
        </div>

        {/* Other quizzes */}
        <div className="main-body-container" style={{ flexWrap: "wrap", gap: 40, justifyContent: "center", width: "100%" }}>
          {quizTypes.filter(q => !q.main).map((quiz) => (
            <div
              key={quiz.key}
              className="country-card"
              style={{
            ...
            >
              <div className="ctd-info" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {quiz.icon}
                  <p className="ctd-title" style={{ fontWeight: "bold", fontSize: 18 }}>{quiz.title}</p>
                </div>
                <p className="ctd-desc" style={{ fontSize: 14, color: "var(--text-color)" }}>{quiz.description}</p>
              </div>
            </div>
          ))}
        </div>
